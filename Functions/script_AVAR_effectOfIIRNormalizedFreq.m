%%%%%%%%%%%%%%%%% script_AVAR_effectOfIIRNormalizedFreq.m %%%%%%%%%%%%%%%%%
%% Purpose:
%   The purpose of this script is to demonstrate the effect of IIR's
%   normalized frequency on AVAR of the filter output.
%
...
iir_filter_order = 3;
list_normalized_cutoff_frequencies = [(0.02:0.02:0.1), (0.2:0.2:0.8)];
number_of_IIR_filters              = numel(list_normalized_cutoff_frequencies);
...
avar_input = fcn_AVAR_favar([input_signal(1:number_of_time_steps); 0],...
             list_of_correlation_intervals);

%% Plot AVAR of I/O of IIR filter
default_color_map = jet(256);
custom_color_map  = default_color_map(1:floor(256/number_of_IIR_filters):256,:);
legend_cell       = cell(number_of_IIR_filters,1);
figure(12345)
clf
...
legend_cell{1} = 'In';
for index_freq = 1:number_of_IIR_filters
    normalized_cutoff_frequency     = list_normalized_cutoff_frequencies(index_freq);
    ...
    %% Estimate AVAR of output of IIR filter
    avar_output = fcn_AVAR_favar([iir_filter_output; 0],list_of_correlation_intervals);
    plot(list_of_correlation_intervals,avar_output,'Color',custom_color_map(index_freq,:),'Linewidth',1.2)
    legend_cell{index_freq+1} = ['$\omega_{n} =$ ' num2str(normalized_cutoff_frequency)];
